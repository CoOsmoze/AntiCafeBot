...
def command_handler(dispatcher):
    ...
    dispatcher.add_handler(MessageHandler(Filters.regex("Моя бронь"), my_order))
    dispatcher.add_handler(CommandHandler("myorder", my_order))

    #new order
    
    new_order_handler = ConversationHandler(
        entry_points=[CommandHandler('neworder', new_order), MessageHandler(Filters.regex("Арендовать комнату"), new_order)],
        states={
            0: [CallbackQueryHandler(pattern = r'@doneorder', callback = count_people)],
